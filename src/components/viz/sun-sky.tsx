...
export function SunSky({
  ...
  if (!Number.isFinite(width) || width <= 0 || size <= 0) {
    return null;
  }
  ...
  const normalised =
    peakAltitude > 0 ? clamp(altitude / peakAltitude, -MAX_DIP, 1) : -MAX_DIP;
  ...
  return (
    <View
      ...
      <View style={[styles.disc, { left: centerX - radius, top: centerY - radius }]}>
        {/* The horizon is drawn by the scrim in the parent, not by the disc. */}
        <SunDisc size={size} showHorizon={false} gradient={gradient} />
      </View>
    </View>
  );
}
